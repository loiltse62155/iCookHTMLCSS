...
CQ_Analytics.Operator.IS_EMPTY="isEmpty";
CQ_Analytics.Operator.IS_NOT_EMPTY="isNotEmpty";
...
addOperator(CQ_Analytics.Operator.IS_EMPTY,"is empty",function(s1,s2){if(s1){return false
}return true
});
addOperator(CQ_Analytics.Operator.IS_NOT_EMPTY,"is not empty",function(s1,s2){if(s1){return true
}return false
});
...
},operate:function(object,property,operator,value,valueFormat){try{if(object){var toEval="";
var op=this.getOperation(operator);
op=op?op:operator;
var objectValue=object[property]?CQ.shared.XSS.getXSSTablePropertyValue(object,property):"";
...
}else{toEval="objectValue "+op+" value"
}var b=eval(toEval);
return b
...
CQ_Analytics.PageDataMgr.loadInitProperties(CQ_Analytics.CCM.getInitialData(CQ_Analytics.PageDataMgr.getName()));
CQ_Analytics.PageDataMgr.init();
CQ_Analytics.CCM.register(CQ_Analytics.PageDataMgr)
...
CQ_Analytics.EventDataMgr.loadInitProperties(CQ_Analytics.CCM.getInitialData(CQ_Analytics.EventDataMgr.getName()));
