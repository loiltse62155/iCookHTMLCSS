...
SNI.Analytics.moduleTrackLS = function (obj,options) {
	...
	if ( options.TargetUrl === "video_play" || ( options.TargetUrl !== "on page interaction" && options.TargetUrl !== options.LocUrl  && options.TargetUrl.split("?")[0].indexOf(document.location.hostname) !== -1 ) ){
		try {
      _satellite.notify("TRACKMOD: "+ JSON.stringify(options),1);
...
// overwrite s.trackModLocalStorage
$(function() { 
  if ( typeof s === "object" ) {
